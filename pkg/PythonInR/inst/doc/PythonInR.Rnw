...
ws$append(c('=SUM(A3:A12)', '=SUM(A3:A12)'))
## create a new sheet (NOTE: Python starts indexing at 0)
ws_plot <- wb$create_sheet(title=plot)
ws_plot$cell(row = 1L, column = 2L)$`_set_value`(3)
ws_plot$cell("C2")$value
wb$save("cars.xlsx")
@
...
def get_range(ws, first_row, first_col, last_row, last_col):
    ...
    return x
")
do.call(rbind, pyCall("get_range", list(ws, 1L, 0L, 10L, 2L), simplify=FALSE))
@
